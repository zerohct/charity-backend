...
export class CampaignsService {
  ...
  async create(body: CreateCampaignDto, file?: Express.Multer.File): Promise<Campaign> {
    ...
    if (base64Image) {
      const matches = base64Image.match(/^data:(image|video|audio|application)\/([a-zA-Z0-9]+);base64,/);
    ...
    async search(query: string, page = 1, size = 10): Promise<{ data: Campaign[]; total: number }> {
      if (!query) {
...
async addMedia(campaignId: number, base64Image: string): Promise<CampaignMedia> {
